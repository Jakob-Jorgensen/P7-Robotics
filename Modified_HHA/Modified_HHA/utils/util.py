...
def getRMatrix(yi, yf):
    if np.isscalar(yf):
        ...
        # Ensure ax is a valid 3D vector
        if ax.shape != (3,):
            ax = np.array([0.0, 0.0, 0.0])
            
        if np.linalg.norm(ax) == 0:
            ax = np.array([1, 0, 0])  # Default axis if yi and yf are collinear
        else:
            ax = ax / np.linalg.norm(ax)

        # Calculate the angle of rotation, using np.clip to handle floating-point errors
        phi = np.degrees(np.arccos(np.clip(np.dot(yi.T, yf), -1.0, 1.0)))

    # Only create rotation matrix if the angle is significant
    if abs(phi) > 0.1:
        phi = np.radians(phi)  # Convert angle to radians

        # Define the skew-symmetric matrix for the cross product
        s_hat = np.array([[0, -ax[2], ax[1]],
                          [ax[2], 0, -ax[0]],
                          [-ax[1], ax[0], 0]])

        # Rodrigues' rotation formula for rotation matrix R
        R = np.eye(3) + np.sin(phi) * s_hat + (1 - np.cos(phi)) * np.dot(s_hat, s_hat)
    else:
        R = np.eye(3)  # No significant rotation, return identity matrix
